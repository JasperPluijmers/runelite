...
public class ClueItemsOverlay extends WidgetItemOverlay {
    ...
    @Inject
    private ClueItemsOverlay(ItemManager itemManager, ClueItemsConfig config, Client client) {
        this.client = client;
        this.config = config;
    ...
    public void renderItemOverlay(Graphics2D graphics, int itemId, WidgetItem itemWidget) {
        ClueItemTier tier = ClueItemTier.highestEnabledTier(itemId, this::isEnabled);
        switch (tier) {
            case NONE:
                return;
            default:
